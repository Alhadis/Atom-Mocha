...
class AtomMocha{
	...
	setup(args){
		...
		this.attachAssets(this.options);
	}
	 ...
	 *
	 * TODO: Add support for "doc" reporter in interactive mode.
	 *
	 * @param {Array} files - Paths loaded by getFiles()
	 * @param {Object} args - Arguments supplied by Atom's test-runner API
	 * @return {Object} opt - An object to pass directly to Mocha
	 */
	getOptions(files, args){
		const {specs, opts} = files;
		const {findBasePath, regexFromString} = require("./utils");
		...
		while(basePath && basePath !== "/"){
			...
			if(fs.existsSync(packagePath)){
				try{
					...
					/** We have options specified in package.json */
					if(data = data["atom-mocha"]){
						Object.assign(opt, data.mocha || {});
						
						/** Load Atom-Mocha's options */
						if(null != data.js)           opt.js           = data.js;
						if(null != data.css)          opt.css          = data.css;
						if(null != data.autoIt)       opt.autoIt       = data.autoIt;
						if(null != data.formatCode)   opt.formatCode   = data.formatCode;
						if(null != data.specPattern)  opt.specPattern  = regexFromString(data.specPattern);
		...
		/** Normalise asset paths */
		for(let name of ["js", "css"]){
			let paths = opt[name];
			
			if(paths){
				
				/** Box any paths supplied as strings or arrays */
				if("string" === typeof paths) opt[name] = {header: [opt[name]]};
				else if(Array.isArray(paths)) opt[name] = {header: paths};
				
				else{
					if("string" === typeof paths.header) paths.header = [paths.header];
					if("string" === typeof paths.footer) paths.footer = [paths.footer];
					if(paths.header || paths.footer) opt[name] = paths;
					else delete opt[name]; /* Torch any pointless values */
				}
			}
			
			/** Resolve asset paths relative to package directory */
			if(this.packagePath && (paths = opt[name]))
				for(let k in paths) if(paths[k])
					paths[k] = paths[k].map(p => path.resolve(this.packagePath, p));
		}
	...
	filterSpecs(paths){
		const specs = [];
		const pattern = this.options.specPattern || /-spec\.(?:coffee|js)$/i;
	...
	run(){
		...
		return run.then(this.attachAssets()).then(_=> new Promise(resolve =>
