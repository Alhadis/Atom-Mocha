...
class AtomMocha{
	...
	getOptions(files, args){
			...
			if(fs.existsSync(packagePath)){
				try{
					...
					if(data = data["atom-mocha"]){
						...
						if(null != data.js)           opt.js           = this.resolveAssets(data.js, this.packagePath);
						if(null != data.css)          opt.css          = this.resolveAssets(data.css, this.packagePath);
		...
		if(opts.length){
			...
			if(mochaOpts.css) opt.css = this.resolveAssets(mochaOpts.css, path.dirname(opts[0]));
			if(mochaOpts.js)  opt.js  = this.resolveAssets(mochaOpts.js, path.dirname(opts[0]));
