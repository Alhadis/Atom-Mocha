...
const {deindent, defineAssertions, flattenList, wait, waitForEvent} =
...
module.exports = {
	...
	/**
	 * Open a list of files relative to current directory.
	 *
	 * @param {...String} paths
	...
	async open(...paths){
		return Promise.all(paths.map(async path => {
			path = resolve(join(...path.split(/[\\/]+/g).filter(Boolean)));
			const textEditor = await atom.workspace.open(path);
			await Promise.race([
				wait(900),
				waitForEvent(textEditor, "did-stop-changing"),
			]);
			return textEditor;
		}));
	},
