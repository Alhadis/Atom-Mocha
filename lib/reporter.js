...
const {addTo, nearest, New, parseKeywords} = utils;
...
class Reporter{
	...
	addResult(test, error){
		...
		error
			...
			: test.pending || addTo(container)(this.createCodeBlock(test.body));
	...
	createErrorBlock(error){
		...
		/** We have an error stack; polish it up a little */
			...
			if(diffEl = this.createDiffBlock(error))
		...
		return addTo(div)(
			New("div", {textContent: title, className: "error-title"}),
			diffEl,
	 ...
	 *
	 * @param {AssertionError} error
	 * @return {Array} output - Array with HTML code for title and stack
	 */
	formatStackTrace(error){
		...
		if(this.options.clipPaths)
			text = text.replace(this.options.packagePath + path.sep, "");
		...
		if(this.options.reverseStackTraces)
			html = html.reverse();
