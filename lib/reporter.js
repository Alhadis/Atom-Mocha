...
const {addTo, nearest, New, escapeRegExp, escapeHTML, jumpToFile, parseKeywords} = utils;

const cssPath = require.resolve("./reporter.css");
const linkSym = Symbol("Atom-Mocha-Elements");
...
class Reporter{
	...
	constructor(runner, options){
		...
		this.element = New("div", {id: "mocha", reporter: this});
		...
		if(this.scrollbarWidth = utils.getScrollbarWidth()){
			...
			const {style} = this.minimal ? this.element : this.report;
	...
	get filter(){ return this.element.dataset.filter || null; }
	set filter(input){
		input = String(input || "").toLowerCase() || null;
		const {dataset}   = this.element;
		const validValues = [null, "passes", "pending", "failures"];
		if(validValues.includes(input) && input !== dataset.filter){
			if(input){
				dataset.filter = input;
				for(const el of this.element.querySelectorAll(`.suite.has-${input}.collapsed`))
					el.classList.remove("collapsed");
			}
			else delete dataset.filter;
		}
	}
	
	get isOffscreen(){
		const classes = this.element.classList;
		return classes.contains("sliding") && (
			classes.contains("offset-left")  ||
			classes.contains("offset-right") ||
			classes.contains("offset-down")  ||
			classes.contains("offset-up")
		);
	}
	...
	set passes(input){
		...
		el.classList.toggle("zero", 0 === input);
	}
	...
	onEnd(){
		...
		if(!this.summary){
			...
			const textContent = total
				? this.runner._abort
					? `Aborted with ${total} tests remaining`.replace(/\b(1 test)s/, "$1")
					: this.options.bail
	...
	onClick(event){
		...
		if(node = nearest(target, ".stack-source.link[data-path]"))
			jumpToFile(node.dataset.path, node.dataset.line, node.dataset.column);
		
		// Individual test
		if(title = nearest(target, ".test-title")){
			parent   = title[linkSym].container;
			isClosed = parent.classList.toggle(hideClass);
			
			if(altKey){
				for(const e of Array.from(parent.parentElement.children)){
					if(!e[linkSym] || !e[linkSym].details) continue;
		...
		// Suite
			...
			if(altKey){
				siblings = Array.from(parent[linkSym].parent.children);
		...
		// Stat-bar filter
		else if(node = nearest(target, "#mocha-stats")){
			const {reporter} = nearest(node, "#mocha");
			const type       = target.id.replace(/^mocha-/, "");
			reporter.filter  = type === reporter.filter ? null : type;
		}
	...
	/**
	 * Handler called when user releases a keystroke.
	 *
	 * @param {KeyboardEvent} event
	 * @this {Reporter}
	 */
	onKeyUp(event){
		if(event.defaultPrevented || this.isOffscreen)
			return;
		
		const isRunning = this.runner.started && !this.finished;
		if(isRunning && "c" === event.key && event.ctrlKey){
			addTo(this.report)
				(New("samp", {className: "echo"}))
				(New("kbd", {textContent: "^C"}));
			this.runner.abort();
		}
	}
	...
	addResult(test, error){
		...
		const prefix    = (this.options.autoIt && "hook" === test.type) ? "In " : "";
		...
		if(!test.pending){
			...
			duration.title += ` (${"okay" === speed ? "medium" : speed})`;
		...
		test.parent[linkSym].results.appendChild(container);
		this.linkElements({container, title, details});
		
		const suiteClasses = test.parent[linkSym].container.classList;
		test.pending && suiteClasses.add("has-pending");
		
		if(error){
			suiteClasses.add("has-failures");
			const el = test.parent[linkSym].container.closest(".suite:not(.has-failures)");
			if(el && this.report.contains(el))
				el.classList.add("has-failures");
		}
	...
	createCodeBlock(src){
		...
		if(this.options.formatCode){
			code.textContent = "";
			src = src.replace(/^async\s+/, "").replace(/^(function\s*)\*/, "$1");
			src = clean(src).replace(/^(\x20{2})+/gm, s => "\t".repeat(s.length / 2));
			for(const node of this.highlight(src))
	...
	createDuration(input = 0, noUnits = false, attr = {}){
		const attrib  = {...attr, className: "duration"};
		...
		Object.defineProperty(span, "value", {
			...
			set(input){
				...
				if(noUnits || input < 1000){
					...
					span.title = `${input} millisecond${1 === +input ? "" : "s"}`;
					...
					span.title = `${input} minute${1 === +input ? "" : "s"}`;
					data.textContent = input;
					unit.textContent = "m";
				}
					...
					span.title = `${input} second${1 === +input ? "" : "s"}`;
					data.textContent = input;
					unit.textContent = "s";
				}
			},
	...
	formatStackTrace(error){
		...
		const stack = error.rawStack.slice();
		...
		return [this.options.escapeHTML ? escapeHTML(title) : title, frames];
	}
	...
	highlight(input){
		...
		for(const line of grammars["source.js"].tokenizeLines(input)){
	...
	/**
	 * Group a list of DOM nodes by embedding a Symbol keyed to references.
	 *
	 * @param {Object} objects
	 * @internal
	 */
	linkElements(objects){
		for(const key in objects){
			const obj = objects[key];
			if(null == obj) continue;
			const copyTo = obj[linkSym] || {};
			obj[linkSym] = Object.assign(copyTo, objects);
		}
	}
	
	
	/**
	 * Keep the bottom-edge of the reporter visible, if close enough.
	 *
	 * @param {HTMLElement} [target=null]
	 * @internal
	 */
	doAutoScroll(target = null){
		if(!this.options.autoScroll) return;
		const targetEdge = (target || this.report.lastElementChild).getBoundingClientRect().top - 16;
		const frameEdge = this.report.getBoundingClientRect().bottom + this.report.scrollTop;
		if(targetEdge <= frameEdge)
	...
	static beforeStart(main){
		...
		if(main.options.formatCode){
			...
			promises.push(atom.packages.activatePackage("language-javascript").then(p => {
				for(const grammar of p.grammars)
					if(grammar.scopeName && !/TreeSitter/i.test(grammar.constructor.name))
