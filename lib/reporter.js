...
class Reporter{
	...
	addResult(test, error){
		...
		error        && suiteClasses.add("has-failures");
	...
	doAutoScroll(){
		...
		const lastChild = this.lastResult || this.report.lastChildElement;
		const lastEdge  = lastChild.getBoundingClientRect().top - 16;
		const frameEdge = this.report.getBoundingClientRect().bottom + this.report.scrollTop;
		if(lastEdge <= frameEdge)
