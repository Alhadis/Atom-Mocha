...
class Reporter{
	...
	constructor(runner, config){
		...
		this.element.addEventListener("click", function(e){
			const title = nearest(e.target, ".test-title");
			if(title){
				title.nextSibling.hidden = !title.nextSibling.hidden;
				e.preventDefault();
				e.stopPropagation();
				return false;
			}
	...
	addResult(test, error){
		...
		addTo(title)(
			...
			this.createDuration(test.duration, true)
		);
 ...
 */
function nearest(node, selector, ignoreSelf){
	let match;
