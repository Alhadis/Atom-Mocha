...
class Reporter{
	...
	onEnd(){
		...
		if(!this.summary){
			...
			const textContent = total
				? `Bailed with ${total} tests remaining`.replace(/(1 test)s/, "$1")
				: "Finished";
	...
	formatStackTrace(error){
		let text = error.stack.split(/^\s+(?=at)/gm);
		...
		html = html.filter(i => i).join("\n").replace(
			/^at\s+((?:[\S\\/]+[\\/])+[^:\n]+):([^:\n]+):([^:\n]+)$/gm,
			(match, ...args) => `<span class="stack-closure">at ${this.formatPath(...args)}`
		).replace(
			/^at\s+(\S+)(\s+.*?)\(([^:\n]+):([^:\n]+):([^:\n]+)\)/gm,
			(match, callee, notes, file, line, col) => {
	...
	/**
	 * Generate HTML code for displaying a source-path in a stack trace.
	 *
	 * @param {String} file
	 * @param {Number} line
	 * @param {Number} col
	 ...
	 * @return {String}
	 */
	formatPath(file, line, col, before = "", after = ""){
		...
		const openingTag = this.options.linkPaths && pathPattern.test(file)
			? `<span class="stack-source link" data-line=${line} data-column=${col} data-path="${file}">`
